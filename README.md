...
The service accepts batched log entries, validates them, stores them durably, and exposes APIs for querying and time-bucketed aggregation. In the included 60-second local benchmark it sustained **21,757 logs/second** under the project's resource limits.
...
- **Committed batch writes**: concurrent requests are coalesced for up to 2 ms (maximum 2,048 rows) and each caller receives `200` only after its rows and rollups commit successfully.
- **One SQL statement per batch**: `jsonb_to_recordset` converts the payload inside PostgreSQL; the same CTE writes `Log`, minute rollups, and second rollups atomically.
- **Source of truth remains `Log`**: unlogged rollup tables are derived caches. If they are empty after a database restart, they are rebuilt from `Log` before the HTTP server starts accepting traffic.
- **Prisma** is used for schema management and parameterized reads. The raw `pg.Pool` is used for high-throughput inserts to avoid per-row ORM overhead.
...
`LogRollup` and `LogSecondRollup` use `(bucket, service, level)` as their primary key and contain pre-aggregated counts. They are derived from `Log`, never a second source of truth.

### Index Rationale

| Index | Purpose |
|---|---|
| Primary key `(timestamp, id)` | A backward B-tree scan gives deterministic `timestamp DESC, id DESC` pagination without maintaining a duplicate descending index. |
| `GIN (attributes jsonb_path_ops)` | Serves containment predicates used by `attr.<key>=<value>` filters. |
| Minute and second rollup primary keys | Keep long-range aggregates small while retaining exact counts at partial-minute boundaries. |

### ID generation

PostgreSQL assigns UUID-shaped IDs from a sequence during the set-based insert. They are unique, avoid per-row JavaScript UUID work, and make the `(timestamp, id)` cursor ordering stable when timestamps collide.
...
- **Indexed containment** — a `jsonb_path_ops` GIN index avoids a full table scan for equality filters.

### Attribute Query Pattern

All `attr.<key>=<value>` filters are compared **as strings**, matching the project spec. The query uses indexed containment and checks the string representation; canonical numeric and boolean forms also match JSON numeric/boolean input:

```sql
WHERE attributes @> jsonb_build_object($1::text, $2::text)
   OR attributes @> jsonb_build_object($1::text, $2::text::numeric)
```
...
- The single JSONB GIN index is the read/write trade-off retained after benchmarking; extra service, level, and message indexes were deliberately removed because their write amplification reduced ingestion throughput.
...
5. The rollup buckets intersecting the cutoff are rebuilt from the remaining source rows, so aggregation stays exact after retention.
...
| Parameter | Value |
|---|---|
| Batch size | 32 logs per request |
| Concurrent requests per wave | 64 |
| Test duration window | 60 seconds |
...
|---|---|---|
| **Ingestion Throughput** | **21,757 logs/second** ✅ | ≥ 15,000 logs/sec |
| **Failed Batches** | **0** ✅ | 0 |
| **Aggregation Query P95** | **89 ms** ✅ | < 1,000 ms |
| **Total Logs Ingested** | **1,306,624** in 60.05 seconds | — |

### Bottlenecks Discovered

1. **Per-request database commits** — a write per HTTP request saturates the single PostgreSQL CPU. Short, bounded coalescing yields one durable set-based write for many callers without acknowledging data early.

2. **Repeated aggregation of raw rows** — grouping millions of source rows competes with ingest. Synchronous minute/second rollups turn common aggregation requests into small cache scans while preserving exact boundary counts.

3. **Write-heavy indexes** — separate service, level, and message indexes lowered ingest capacity. The final design keeps only the pagination primary key and the required JSONB attribute index.
...
| `jsonb_to_recordset()` batch insert | One parameterized SQL statement instead of N individual INSERT statements |
| Atomic minute and second rollups | Immediate read-after-write visibility and fast exact aggregates |
| Sequence-based UUID-shaped IDs in SQL | Avoided per-row JavaScript UUID work |
...
| `work_mem=32MB` | Avoids lossy bitmap scans for JSONB attribute queries |
...
5. **Windows Docker Desktop overhead** — The measured throughput (21,757 logs/sec) reflects Docker Desktop on Windows. Native Linux Docker is expected to perform significantly better due to the absence of WSL2 I/O virtualization.
